...
if [ "$USE_SIM_TIME" = "True" ]; then
    ...
    tmux send-keys "ros2 launch dji_captain dji_captain.launch" C-m
fi
...
if [ "$USE_SIM_TIME" = "True" ]; then
    tmux send-keys "ros2 launch str_json_mqtt_bridge mqtt_bridge.launch robot_name:=$ROBOT_NAME domain:=air realsim:=simulation" C-m
else
