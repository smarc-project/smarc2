...
| --- | ---| ---|
| 'init_auv_search' | InitAUVSearch | It initiates the **PGM**. It should be requested when the search planning is about to initiate. It needs the GPS ping and the initial drone position in any frame connected to Quadrotor's odom. It can be called several times in a single simulation since the planner and the grid map are reinitialized when this service is requested|
