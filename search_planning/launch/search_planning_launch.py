...
def generate_launch_description():
    ...
    # --- choose path planner 
    'path_planner': 'spiral', # 'spiral', 'greedy', 'astar', 'apf'

    # --- common parameters to all path planners
    'initialization.time_delay': float(4),
    'flight_height': 6.0,
    'camera_fov': 82.0,
    'look_ahead_time': float(1), #lat [s] × velocity sets the waypoint distance threshold. Recommended values (normalization = 1): Spiral = 0.6, Greedy = 3, A* = 2.
    ...
    'arf.k_repulsive': 25, # k constant for repulsive potential field (bigger -> neighbouring cells will repulse more)
    'arf.goal_distance_factor': 100, # factor to determine how far is the goal (bigger factor -> smaller acceleration by resultant of forces -> closer goal). Think of it as mass
    ...
    'arf.horizon_radius': float(30), # arf planner picks the highest-probability cell within radius x; -1 uses full workspace

    # --- SAM configs
    'sam.initial_state.position': [float(30), float(30)], # initial position to which sam will teleport (in odom). User-defined, only useful in simulation
    ...
    'grid_map.workspace.center': [100.0, 50.0],
    'grid_map.workspace.width': float(120.0),
    'grid_map.workspace.height': float(120.0),
    'grid_map.workspace.resol': float(2), 
    'grid_map.workspace.variance': 3.0, # gaussian variance considered in the prior distribution (initial grid map)
