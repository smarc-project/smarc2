...
def test_before_after():
    ...
    global sut
    sut = ONNXManager("../resource/SAMSimple.onnx")
...
def test_get_control_dummy():
    control = sut.get_control(np.zeros((1, 13), dtype=np.float32))
...
def test_get_control_scaled():
    control = sut.get_control_scaled(np.ones((1, 13), dtype=np.float32) / 2)
