...
class EvoloMovePath():
    ...
    def _on_goal_received(self, goal_request: dict) -> bool:
        ...
        for wp in waypoints:
            ...
            target_speed = speed
            target_tol = float(max(10,wp_params['tolerance']))
    ...
    def _loop_inner(self) -> bool | None:
        ...
        target_position = self.target_list[self.target_index].p
        self.distance_to_target = self.calculate_distance(self.robot_position, target_position)
        if(self.distance_to_target < self.target_list[self.target_index].tol):
            self.target_index += 1
        ...
        targetYaw = Float32()
        dx = target_position.pose.position.x - self.robot_position.pose.position.x
        dy = target_position.pose.position.y - self.robot_position.pose.position.y
